...
%             - sources are already separated, helping future more precise annotations
%             - large dataset, affording certain decisions that would be otherwise restricted
%             - Can be standard for this task after improvements; Andrei et al. cite lack of if
% TODO: [text] mention ovl percentages in training and test
Previous works on detecting overlapping speech in conversations have used data from meetings in datasets like AMI, NIST RT, ICSI, and others.  % TODO: T [ref] to meeting datasets
While they are appropriate, and perhaps the flagship scenario for evaluation,
these datasets extra extra challenges due to inconsistent and difficult recording conditions.
Previous works have utilized only a subset of these datasets,
...
This subset was not enough to appropriately train a DCNN with the different strategies to tackle class-imbalance.
We decided to instead perform our study using the Fisher English Corpus (Part 1),  % TODO: T [ref] Fisher paper
which
% consists of 5,850 telephone based conversations (amounting to more than 960\,hours) and
is predominently used in conversational and large vocabulary speech recognition research.  %TODO: T [ref] papers using Fisher
...
However, we only roughly used roughly 200~hours of the dataset during trainings
...
A more important problem with the dataset is the relative imprecision in annotation of the utterance boundaries.
...
Rebalancing the dataset that is seen by the system during training is the most common approach used to tackle the impact of imbalanced classes,
by either oversampling from the minority classes or undersampling from the majority classes.
When the available dataset is limited, oversampling is the most prevelant method,
...
we instead chose to only evaluate majority undersampling for one of our experiments.
...
This may result in situations where a neural network gets stuck in a steep local minima that is irrelevant, or even disadvantageous to double-talk detection.
...
% ----------------------------------------------------------------------------------------- OTHER -
\subsection{Other}
Moving the decision threshold based on class priors is another method to tackle class imbalance.
...
However, for stochastic gradient descent,
...
Temporal smoothing improved $P^{(no)}$ and $P^{ov}$ by either removing impossibly short segments from the raw predictions or by filling in certain gaps (Figure X),  % IDEA: T [fig] true, raw, smoothed preds for baseline
...
% We doubled the number of passes for this experiment to avoid any under-fitting of the single-speaker class.
